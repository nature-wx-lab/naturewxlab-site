...
SNS共有カードは、タイトルを「植物・メダカ・自然観察を、気象の視点で読み解く」、画像内メッセージを「天気を味方に、自然と暮らす」とします。カードには「気象予報士パパの自然研究所」の日本語名も併記し、青空、木漏れ日、水面、植物、メダカを一枚で伝えるビジュアルとロゴを組み合わせます。Xなどの既存画像キャッシュと区別するため、現行画像は `og-image-20260722-v4.jpg` の固有URLで配信し、全ページのOpen Graph／X Card画像と代替文を同じ内容へ揃えます。以前の共有カード画像は、既存投稿の参照切れを避けるため旧URLのまま保持します。
...
2. 新しい共有画像を、既存ファイルを上書きしない版付きの新ファイル名で `site/assets/images/` へ追加する。
3. 全ページの `og:image`、`twitter:image`、画像altを新URLへ揃え、`scripts/verify_site.py` の期待値、画像allowlist、寸法・SHA-256契約も同時に更新する。
