...
SNS共有カードは、タイトルを「植物・メダカ・自然観察を、気象の視点で読み解く」、画像内メッセージを「天気を味方に、自然と暮らす」とします。Xなどの既存画像キャッシュと区別するため、現行画像は `og-image-20260722-v2.jpg` の固有URLで配信し、全ページのOpen Graph／X Card画像と代替文を同じ内容へ揃えます。以前の共有カード画像は、既存投稿の参照切れを避けるため旧URLのまま保持します。
