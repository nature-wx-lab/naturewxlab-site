...
VisionのFUTUREは、今ある情報・ツールの拠点から、アバターで参加する仮想世界（メタバース）の「オンライン上の街」と、リアルなイベント・体験・対面交流の両方へ広がり、仮想と現実を結ぶ自然の交流拠点へ向かう01〜04の道のりとして示します。オンライン上の街では、施設やイベントへの参加、経験や情報の直接共有、お気に入りの一品や一匹との出会い・交換を構想します。04だけを控えめに強調し、支援・収益を次の活動へ還元する `CONTINUITY` は第5段階に見えない独立セクションとして配置します。現在地と構想を分けて伝える `PROMISE` と公開ツールへの導線は維持します。

この更新では既存の青空画像をそのまま使用します。検討中の構想イラストは、内容とモバイル表示を見直してから別の更新で扱うため、公開ファイルと画像allowlistには追加しません。NOW左列はPCで14pxだけ上げ、見える書き出し位置を右側カードの上端に揃えます。880px以下では補正を解除して自然な一段組みに戻します。FUTURE導入文の「広がっていく」は語中で分断せず、一まとまりで次行へ送ります。HOW TO CHOOSE導入文は前半2文と後半2文の2段落とし、1160px以上では各段落を1行で表示します。全ページのCSS参照は `v=20260717-6` です。
