...
この更新では既存の青空画像をそのまま使用します。検討中の構想イラストは、内容とモバイル表示を見直してから別の更新で扱うため、公開ファイルと画像allowlistには追加しません。NOW左列はPCで14pxだけ上げ、見える書き出し位置を右側カードの上端に揃えます。880px以下では補正を解除して自然な一段組みに戻します。FUTURE導入文の「広がっていく」は語中で分断せず、一まとまりで次行へ送ります。全ページのCSS参照は `v=20260717-5` です。
