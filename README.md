...
2026-07-16にOpenAIの画像生成機能で作成した実写調のイメージ画像を、既存デザインの余白とタイポグラフィを損なわない3か所だけに使用します。実在の運営者、庭、飼育容器、観察記録を撮影した写真ではありません。各画像には見える形で `AI生成イメージ` と明記し、外部画像URLを読み込まずローカル配信します。

- `editorial-garden-ai-20260716.jpg`：雨上がりの自然な家庭の庭。HomeのOUR APPROACH末尾に掲載。1200×800 JPEG。
- `editorial-medaka-biotope-ai-20260716.jpg`：水草のある屋外容器を泳ぐメダカ。AboutのORIGINに掲載。1200×800 JPEG。
- `editorial-sky-ai-20260716.jpg`：白い雲と遠い山並みのある穏やかな空。VisionのNOW導入部に掲載。1200×614 JPEG。

生成時は自然光、控えめな彩度、実物らしい質感を共通条件とし、人物、住所情報、読めるラベル、第三者ロゴ、透かし、過度な演出を除外しました。公開用JPEGはスマートフォンでも過度な転送量にならない1200px幅へ最適化したうえで、寸法・形式・SHA-256・許可する最小限の画像メタデータを検査スクリプトで固定し、HTMLでは元の縦横比を保ったまま表示します。Toolsは既存の初期画面画像を優先し、Policyは可読性と説明責任を優先して装飾写真を追加しません。
