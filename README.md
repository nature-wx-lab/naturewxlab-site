...
noteが正規URLの古いカード情報を再利用する場合だけ、再取得用URL `https://naturewxlab.com/?note-card=20260723` を使います。クローラーにはquery付きURLのまま新しいOpen Graph／note Cardを返し、実ブラウザでは `navigation.js` が該当する1パラメータだけを `history.replaceState` で除去します。ページを再読込・転送せず、他のqueryとfragmentを保持するため、アドレスバーとお気に入りは正規URLへ戻り、カードの再取得キーは維持されます。このURL正規化を含む全6 HTMLのnavigation script参照は `v=20260723-2` とし、既存ブラウザとCloudflareの旧JavaScript cacheを再利用させません。
...
- Instagram：Meta公式の[Instagramブランドリソース](https://about.meta.com/brand/resources/instagram/instagram-brand/)で配布される黒色グリフの形状とファイルを維持し、同じ公式配布パックに白色グリフとグラデーショングリフが含まれることを確認したうえで、サイト側のCSSだけで白色表示とピンク主体・紫・オレンジ・黄色のグラデーション背景を適用する。アプリアイコン画像そのものは複製せず、リンク先の識別用途に限定する
